#First, we'll need to import the os module, which has functions for interfacing with the operating system.
import os
# We should Import the Python time module, which allows us to get the current time.
import time
...
border = ""
linux = 0
#creating a vehicle class with public attributes like type,plate and entry time
class Vehicle:
...
# A space class is developed with methods for adding, removing, retrieving vehicle information, and checking whether a specific vehicle is there or not.
class Space:
    def __init__(self):
        self.vehicle = None
        self.occupied = False
...
#This method is created to print a specific row
def print_row(row):
    output = ""
    output += "|"
...
#This method displays all of the available spots at a specific point in time.
def display_lot():
...
#This method is used to view a parked vehicle in a specific row
...
#The display_space_selection() method displays parked vehicles and returns the total number of spaces available.
def display_space_selection(row):
    ...
    print(output)
    return space_count
#The below method is used to park a vehicle
...
#The below method is used to access all of a vehicle's details.
...
def read_config():
    ...
    config = open('PythonMiniProject.txt', 'r')
    while True:
